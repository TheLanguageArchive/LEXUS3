...
<!--
    Create a tree representation of the .txt file as parsed by Chaperon, using the schema
    derived from the .typ file using Chaperon and some XSLT translations.
-->
...
<!--    <xsl:include href="create-LexiconSchema.xslt"/>-->
    ...
    <xsl:template match="toolbox-import">
        <Lexicon>
            <lexus:schema>
                <xsl:copy-of select="lexiconSchema"/>
            </lexus:schema>
    ...
    <xsl:template match="marker">
        ...
        <xsl:variable name="schemaId" select="key('components', $attrName)[1]/@id"/>
            ...
            <xsl:when test="key('components', @name)/component">
                ...
                <xsl:choose>
                    <xsl:when test="$nextMarkerAncestorNames = $attrName or $parent eq ''">
                        ...
                        <component ref="{$schemaId}" name="{concat($schemaName, 'Group')}"
                            marker="{@name}">
<!--                            <feat att="mdf:{@name}" value="{@value}"/>-->
                            <data-category ref="{$schemaId}" name="{$schemaName}" marker="{@name}">
                        ...
                        <xsl:call-template name="process-possible-sibling">
                            ...
                            <xsl:with-param name="nextMarker"
                                select="following-sibling::marker[position() eq 1 + count($children//data-category)]"
                            />
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise>
...
<!--                <feat att="mdf:{@name}" value="{@value}"/>-->
    ...
    <xsl:template name="process-possible-sibling">
        ...
        <xsl:variable name="nextMarkersAncestors"
            select="key('components', $nextMarker/@name)/ancestor::component/@marker"/>
    ...
    <xsl:template match="@name | @value" mode="dc"/>
