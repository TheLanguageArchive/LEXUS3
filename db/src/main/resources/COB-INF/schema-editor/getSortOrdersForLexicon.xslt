    ...
    <xsl:template match="lexus:update-lexicon-for-updated-schema">
        <xsl:copy>
            ...
            <lexus:text>
                ...
                let $sortorder-ids := distinct-values(collection('<xsl:value-of select="$lexica-collection"/>')/lexus[@id = '<xsl:value-of select="$lexicon-id"/>']/meta/schema//container/@sort-order)
