...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <object>
            <xsl:apply-templates select="data/lexus:get-listview/lexus:result"/>
            <object key="status">
                <xsl:choose>
                    <xsl:when test="data/lexus:get-listview/lexus:result[@success = 'true']">
                        <true key="success"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <false key="success"/>
    ...
    <xsl:template match="lexus:result">
        <object key="result">
            <string key="entry">
                <xsl:apply-templates select="//display:lexicon/lexical-entry/xhtml:html" mode="encoded"/>
            </string>
            <array key="lius"><xsl:apply-templates select="view//data"/></array>
        </object>
    </xsl:template>
    
    <!--
        Generate data for a data element in the view 
        <data isBranch="false" type="data category" name="lexeme" id="uuid:2c9090a2134ee53d01136379320106bd"/>
        - - >
        {"id":"NmU1ZjBkMzYtYmRkNy00ODMwLTljNWQtOWYwMTY3NjY2MDQ5","value":"a dnyaa","schemaElementId":"MmM5MDkwYTIxMzRlZTUzZDAxMTM2Mzc5MzIwMTA2YmQ=",
        "label":"lexeme","notes":null}
    -->
    <xsl:template match="view//data">
        <xsl:variable name="schema-ref" select="@id"/>
        <xsl:apply-templates select="/data/lexus:get-lexical-entry/lexus:result/result/lexical-entry//data[@schema-ref eq $schema-ref]"/>
    </xsl:template>
